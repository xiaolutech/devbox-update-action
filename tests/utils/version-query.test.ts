...
import { VersionQueryService, createVersionQueryService } from '../../src/utils/version-query';
import { NetworkError, ValidationError } from '../../src/types';
import { ParsedPackage } from '../../src/utils/package-parser';
...
describe('VersionQueryService', () => {
  ...
  describe('getLatestVersion', () => {
    ...
    it('should throw ValidationError for response without version', async () => {
      ...
      await expect(service.getLatestVersion('nodejs')).rejects.toThrow(ValidationError);
    });

    it('should throw NetworkError for API failure', async () => {
      mockedAxios.get.mockRejectedValueOnce(new Error('Network error'));
      
      await expect(service.getLatestVersion('nodejs')).rejects.toThrow(NetworkError);
    });
  });
  ...
  describe('getPackageInfo', () => {
    ...
    it('should throw NetworkError for API failure', async () => {
      ...
      await expect(service.getPackageInfo('nodejs')).rejects.toThrow(NetworkError);
    });
  });
