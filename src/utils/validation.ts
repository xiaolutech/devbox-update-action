...
import { type DevboxConfig, ValidationError } from "../types";
 ...
 */
export function validateDevboxConfig(
	config: unknown,
): asserts config is DevboxConfig {
			...
			for (let i = 0; i < shell.init_hook.length; i++) {
				if (typeof shell.init_hook[i] !== "string") {
					throw new ValidationError(
						`Shell init_hook at index ${i} must be a string`,
					);
				}
			}
		}
		...
		if (shell.scripts !== undefined) {
			...
			for (const [key, value] of Object.entries(scripts)) {
				if (typeof value !== "string" && !Array.isArray(value)) {
					throw new ValidationError(
						`Shell script "${key}" must be a string or array`,
					);
				}
				if (Array.isArray(value)) {
					for (let i = 0; i < value.length; i++) {
						if (typeof value[i] !== "string") {
							throw new ValidationError(
								`Shell script "${key}" at index ${i} must be a string`,
							);
						}
					}
				}
...
export function validateForSerialization(config: DevboxConfig): void {
	try {
		...
		throw new ValidationError(
			`Configuration cannot be serialized: ${error instanceof Error ? error.message : "Unknown error"}`,
		);
	}
}
